...
function AdminDeliverdPreorder() {
    ...
    const getPreOrders = async () => {
        try {
        const { data } = await axios.get(`/api/v1/auth/preorder-preproduct-deliverd/${params.preproduct}`);
        setPreorders(data);
        } catch (error) {
        console.log(error);
        }
    ...
    const exporttocsvfunc = async()=>{
        ...
        return await commonrequest("GET",`${BASE_URL}/api/v1/preproduct/preorder-export-deliverd/${params.preproduct}`,"");
    }
...
export default AdminDeliverdPreorder